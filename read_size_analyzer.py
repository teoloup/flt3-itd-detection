        ...
        try:
            ...
            if 'itd_signatures' in self.analysis_results:
                ...
                for class_name in classes:
                    ...
                    class_labels_with_ranges.append(f"{class_name}\n({range_str}bp)")
                
                bars = ax4.bar(range(len(classes)), counts, color=['red', 'orange', 'green', 'yellow', 'blue', 'purple', 'brown'])
                ax4.set_xlabel('Size Class')
                ax4.set_ylabel('Read Count')
                ax4.set_title('Reads by Size Class')
                ax4.set_xticks(range(len(classes)))
                ax4.set_xticklabels(class_labels_with_ranges, rotation=45, ha='right', fontsize=9)
